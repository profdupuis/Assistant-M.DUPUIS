...
from rgpd import (
    ...
    delete_student, download_latest_worm, download_worm, export_flags_graves,
    export_worm, purge_old_logs, prepare_audit, PURGE_DAYS
)
...
engine_log = create_engine(app.config["DATABASE_URL_LOG"], future=True) # 🔒 moteur dédié au logging (app_log)
...
def moderate_API(conversation: str) -> dict:
    ...
    try:
        ...
        return resp.results[0].categories.to_dict()  # Conversion en dict des catégories
...
def moderation(user_msg, reply: str) -> dict:
    ...
    if flags.get("error", False):
        ...
        return True,flags  # Bloquer si l'API échoue
    ...
    return {"blocked": False,"reason": flags}  # Contenu autorisé
...
def api_message():
    ...
    if moderation_result["blocked"]:
        app.logger.warning(f"Conversation modérée pour l'élève {session.get('student_id')}: {moderation_result['reason']}")
        return jsonify({"reply": "Conversation modérée. Veuillez reformuler."})
...
def upload_scenario():
    ...
    with engine.begin() as cn:
        ...
        scenario_id = cn.scalar(text("""
            INSERT INTO scenarios (name, class_name, content, is_active, matiere, prof_id)
            VALUES (:name, :cls, :content, true, :mat, :pid)
            RETURNING id
        """), {
            "name": nom_fiche,
            "cls": class_name,
...
def dashboard_rgpd():
    ...
    if not verifier_integrite_worm(engine):
...
def export_flags_graves_route():
    ...
    """
    return export_flags_graves(engine)
...
def delete_student_route(student_id):
    '''
    Supprime un élève de la base de données via son identifiant.

    - Vérifie d'abord que l'utilisateur est connecté en tant qu'administrateur.
    - Tente de supprimer l'élève correspondant au student_id fourni.
    - Si la suppression réussit, affiche un message de succès.
    - Si aucun élève n'est trouvé ou en cas d'erreur, affiche un message d'erreur.
    - Redirige ensuite vers le dashboard RGPD.

    Méthode protégée par la session admin.
    '''
    delete_student(engine, student_id)
...
def dashboard_eleve():
    ...
    with engine.connect() as cn:
        ...
        if selected_class and selected_scenario:
            if student_id:
                # Si un élève est sélectionné ➔ stats individuelles
                ...
                # Si pas d'élève ➔ evolution moyenne classe + stats moyennes
                evolution = get_evolution_par_scenario()
...
def mon_dashboard():
    ...
    with engine.connect() as conn:
        # 🔥 Charger TOUS les scénarios possibles pour la classe
        ...
        scenarios_disponibles = [{"id": str(r["id"]), "nom": r["name"]} for r in results]
...
def changer_scenario():
    ...
    session["ANS_JSON"] = ans_json
    session["CAT_JSON"] = cat_json
    session["active_scenario_id"] = nouveau_scenario
    session["exo_valide"] = done_refs
    session["has_feedback"] = feedback_exists

    flash("✅ Scénario changé avec succès.", "success")
    return redirect(url_for("mon_dashboard"))
