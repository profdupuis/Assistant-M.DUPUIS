...
from utils.session_utils import (
    ...
    clean_temp_folder,
    latest_scenarios_without_feedback
)
...
def latest_scenarios_without_feedback(engine, class_name, student_id, limit=3):
    ...
    ceux pour lesquels <student_id> possède déjà un feedback.
    """
    sql = """
        WITH last AS (
...
def format_content_latex(content: str) -> str:
    """
    Prend un contenu markdown (texte + ```python blocs) et génère du LaTeX
    avec lstlisting pour le code, et du texte normal pour les maths.
    """
    if not content:
        return ""
    ...
    for i, part in enumerate(parts):
        if i % 2 == 0:
            # Texte normal : on laisse les \(...\), \[...\] intacts
            # mais on échappe les autres caractères LaTeX
            latex += sanitize_tex_preserving_math(part.strip()) + "\n\n"
        else:
            # Bloc de code Python
...
def sanitize_tex_preserving_math(text: str) -> str:
    r"""
    Échappe les caractères LaTeX sauf dans les blocs math \(...\), \[...\], ou $$...$$.
    Convertit aussi les **gras** en \textbf{} hors des blocs math.
    """

    # Regex pour détecter les blocs math LaTeX
    math_pattern = re.compile(r'(\\\(.+?\\\)|\\\[.+?\\\]|\$\$.+?\$\$)', re.DOTALL)

    # Découpe texte en blocs math / non math
    parts = math_pattern.split(text)

    sanitized_parts = []
    for i, part in enumerate(parts):
        if i % 2 == 1:
            # Bloc math → on ne touche pas
            sanitized_parts.append(part)
        else:
            # Texte hors math → échappe les caractères LaTeX et convertit les gras
            part = re.sub(r'([%$&#_^])', r'\\\1', part)  # échappe LaTeX sauf {} (utiles pour \textbf{})
            part = re.sub(r'\*\*(.+?)\*\*', r'\\textbf{\1}', part)  # **gras**
            part = part.replace("\n", " \\\\\n")  # newline → saut de ligne LaTeX
            sanitized_parts.append(part)
...
def build_conversation_pdf():
    ...
    for msg in history:
        ...
        if role == "exo":
            ...
            sections[current_exo].append((role, sanitize_tex_preserving_math(content)))
...
"""
    ...
    tex += f"\\textbf{{Nom de la fiche}} : {sanitize_tex(scenario_name)}\n\n\\bigskip\n"
    ...
    try:
        ...
        app.logger.error(f"Erreur PDF conversation : {e}")
        return None
