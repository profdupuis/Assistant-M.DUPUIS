...
WORM_EXPORT_DAYS = 15  # Export worm et purge tous les logs de moins de WORM_EXPORT_DAYS jours
PURGE_DAYS = 15 
...
def verifier_integrite_worm(engine, generate_report=True):
    ...
    for log in logs:
        # ⚠️  Ne casse pas la verification chaîne sur les lignes anonymisées
        if log["user_id"] == "ANONYMISED":
...
# 📥 Anonymise, Export dernier WORM et delete 
def archive_and_purge(engine, days=WORM_EXPORT_DAYS):
    """
    1. Anonymise → 2. Export WORM → 3. Delete
    Le tout en transaction.
    """
    
    try:
        CUT_SQL = f"now() - interval '{days} days'"


        now = datetime.now(timezone.utc).date()
        ts = now.strftime("%Y-%m-%d_%H-%M-%S")
        
        csv_file = f"chatlogs_worm_{ts}.csv"
        sha_file = csv_file + ".sha256"
        zip_file = f"chatlogs_worm_{ts}.zip"
        ...
        with engine.begin() as cn:        # ➊ transaction
            # 1. anonymiser + lock
            cn.execute(text("LOCK TABLE chat_logs IN SHARE ROW EXCLUSIVE MODE"))
            anon = cn.execute(text(f"""
                UPDATE chat_logs
                SET    user_id = 'ANONYMISED'
                WHERE  ts < {CUT_SQL}
                  AND  user_id <> 'ANONYMISED'
            """))

            # ✅ Anonymiser les réponses dans attempts 
            cleared = cn.execute(text(f"""
                UPDATE attempts
                SET given_answer = NULL
                WHERE ended_at < {CUT_SQL}
            """))

            
            # ✅ Effacer feedback aussi  ? 
            
            

            # 2. exporter WORM (déjà anonyme)
            rows = list(cn.execute(text(f"""
                SELECT id, ts, user_id, prompt, completion,
                       flags, model, prev_hash, this_hash
                FROM   chat_logs
                WHERE  ts < {CUT_SQL}
                ORDER  BY ts ASC
            """)))
            ...
            with open(csv_path, "w", newline='', encoding="utf-8") as f:
                w = csv.writer(f); w.writerow(["id", "ts", "user_id", "prompt", "completion", "flags", "model", "prev_hash", "this_hash"])
                w.writerows(rows)
            ...
            # 3. supprimer
            deleted = cn.execute(text(f"""
                DELETE FROM chat_logs WHERE ts < {CUT_SQL}
            """))

            cn.execute(text("""               -- historique purge
                INSERT INTO logs_purges(purge_date, nb_logs_deleted, nb_attempts_anonymized)
                VALUES(now(), :del, :anon)
            """), {"del": deleted.rowcount, "anon": anon.rowcount})
            ...
            lines = [
                ...
                f"Nombre de logs anonymisés : {anon.rowcount}",
                f"Nombre d'attempts anonymisés : {deleted.rowcount}",
                f"Nombre de réponses anonymisées (attempts) : {cleared.rowcount}"
            ]
            generate_report_sha256(filename, lines, "purges") 

        flash(f"✅ Export WORM + purge : {len(rows)} logs archivés, "
          f"{anon.rowcount} anonymisés, {deleted.rowcount} supprimés.", "success")
...
def export_flags_graves(engine):
    ...
    # Préparation du rapport texte + scellage   
    now = datetime.now(timezone.utc)
    now_str = now.strftime("%Y-%m-%d_%H-%M-%S")
    ...
    for record in results:
        flags = [k for k, v in (record['flags'] or {}).items() if v]
        lines.append(f"- {record['ts'].strftime('%d/%m/%Y %H:%M')} | {record['user_id']} | {', '.join(flags)}")
        lines.append(f"  Prompt : {record['prompt'].strip()[:200]}{'...' if len(record['prompt']) > 200 else ''}")
        lines.append("")
...
def purge_old_logs(engine,days=15):
    """    
...
def export_logs_eleve_csv(engine, student_id):
    """
    Exporte les échanges de l'élève en CSV, en appliquant les règles suivantes :
    - Ajoute '🔒 modéré' si le message a été modéré (flags contient au moins un True)
    - Ajoute '⚠️ erreur système' si flags["error"] == True
    - Garde la réponse originale sinon

    Retourne un objet Flask `Response` téléchargeable.
    """
    ...
    with engine.connect() as cn:
        logs = cn.execute(text("""
            SELECT ts, prompt, completion, flags
            FROM chat_logs
            WHERE user_id = :sid
            ORDER BY ts
    ...
    si = StringIO()
    writer = csv.writer(si)
    writer.writerow(["timestamp", "prompt", "completion"])


    # Partie 1 : logs
    for ts, prompt, completion, flags in logs:
        if flags and isinstance(flags, dict):
            if flags.get("error"):
                completion_finale = "⚠️ erreur système"
            elif any(flags.values()):
                completion_finale = "🔒 modéré"
            else:
                completion_finale = completion or ""
        else:
            completion_finale = completion or ""

        writer.writerow([ts.strftime("%Y-%m-%d %H:%M:%S"), prompt, completion_finale])
        
    # Partie 2 : feedbacks
    with engine.connect() as cn:
        feedbacks = cn.execute(text("""
            SELECT scenario_id, feedback
            FROM feedbacks
            WHERE student_id = :sid
        ...
        if feedbacks:
            writer.writerow([])
            writer.writerow(["--- FEEDBACKS FINAUX ---", "", ""])
            for scenario_id, feedback in feedbacks:
                writer.writerow(["📘 Fiche " + str(scenario_id), "Feedback", feedback])        
    ...
    return Response(
        output,
        mimetype="text/csv",
        headers={"Content-Disposition": f"attachment; filename=logs_{student_id}.csv"}
    )
